...
public class MatchingService {
	...
	public List<Teacher> getTeachersBySubject(Long studentId) {
		...
		// 해당 교과목에 맞는 선생님을 가져오고, 이미 매칭된 선생님은 제외
		List<Teacher> teachers = teacherRepository.findBySubject(studentSubject);
		
		// 매칭된 선생님 제외
		teachers = teachers.stream()
				.filter(teacher -> !isTeacherMatched(teacher.getId()))
				.collect(Collectors.toList());

		return teachers; // 해당 subject의 선생님 목록을 조회
	}
    ...
    public boolean isTeacherMatched(Long teacherId) {
    	// 선생님이 승인된 매칭이 있는지 확인
    	return matchRepository.existsByTeacherIdAndMatchApproved(teacherId, true);
    }
