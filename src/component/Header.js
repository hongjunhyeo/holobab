...
function Header() {
    ...
    const handleStorageChange = () => {
      ...
      if (token) {
        try {
          ...
          const stringId = decodedToken.sub;
  ...
  const onClickAdminPage = () => {
    navigate("/admin/memberManagement"); // 관리자 페이지로 이동
  };
      ...
      <nav className="banner-nav">
        <ul className="banner-nav-list">
          <li className="banner-nav-item">
            <span className="banner-text" onClick={() => onClickNavigate("/FieldInternship")}>현장실습학기제</span>
            <ul className="dropdown-menu">
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/FieldInternship")}>현장실습학기제란?</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/OperationModel")}>운영 모형</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/FieldInternshipOverview")}>한 눈에 보는 현장실습학기제</span></li>
            </ul>
          </li>
          <li className="banner-nav-item">
            <span className="banner-text" onClick={() => onClickNavigate("/CentralSupport")}>중앙지원센터</span>
            <ul className="dropdown-menu">
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/CentralSupport")}>중앙지원센터 소개</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/CentralSupportProgram")}>중앙지원센터 사업</span></li>
            </ul>
          </li>
          <li className="banner-nav-item">
            <span className="banner-text" onClick={() => onClickNavigate("/Library")}>자료실</span>
            <ul className="dropdown-menu">
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/Manual")}>매뉴얼/서식</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/CaseStudies")}>운영 사례</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/Video")}>영상자료</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/Other")}>기타자료</span></li>
            </ul>
          </li>
          <li className="banner-nav-item">
            <span className="banner-text" onClick={() => onClickNavigate("/Information")}>정보마당</span>
            <ul className="dropdown-menu">
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/Notice")}>공지사항</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/Gallery")}>갤러리</span></li>
            </ul>
          </li>
          <li className="banner-nav-item">
            <span className="banner-text" onClick={() => onClickNavigate("/Board")}>게시판</span>
            <ul className="dropdown-menu">
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/Form")}>양식</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/Reference")}>참고자료</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/FAQ")}>F&A 자주 묻는 질문</span></li>
              <li className="dropdown-item"><span onClick={() => onClickNavigate("/FAQ")}>F&A 시스템 건의사항</span></li>
            </ul>
          </li>
        </ul>
      </nav>
          ...
          userRoles.includes("ROLE_STUDENT") || userRoles.includes("ROLE_PROFESSOR") || userRoles.includes("ROLE_TEACHER") ? (
            <>
              {isApproval ? (
                ...
                <p className="isApprovalWait" >승인 대기 중</p> // 승인 대기 중 표시
              )}
              <button onClick={onClickLogout}>로그아웃</button>
              <p className='Header-string-id'><span class="string-id">{stringId}</span>님 환영합니다!!</p>
            </>
            ...
            <>
              ...
              <button onClick={onClickLogout}>로그아웃</button>
              <p className='Header-admin-mode'><span class="admin-mode">관리자 모드입니다!</span></p>
            </>
          ) : (
            <div>권한 없음</div>
          )
