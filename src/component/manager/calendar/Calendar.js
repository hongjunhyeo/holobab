...
const Calendar = () => {
  ...
  //접근 제한
  const isNotStudent = userRole && !userRole.includes('ROLE_STUDENT'); 
  ...
  return (
    <div className="calendar-container">
        ...
        <ul>
          {filteredEvents.map((event) => (
            <li key={event.id} onClick={() => handleListEventClick(event)}>
              ...
              {event.startDate} - {event.endDate} ◆ {event.writerName}
            </li>
          ))}
        </ul>
      </div>
      ...
      {/* 일정 추가 모달 */}
      {isLoggedIn && isNotStudent &&showAddModal && (
        <Modal
          isOpen={showAddModal}
      ...
      {/* 일정 수정 모달 */}
      {isLoggedIn && isNotStudent &&showEditModal && selectedEvent && (
        <Modal
